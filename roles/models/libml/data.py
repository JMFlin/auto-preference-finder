...
flags.DEFINE_string('dataset', 'fixmatch_train1@1', 'Data to train on.')
...
class DataSets:
    def __init__(self, name, train_labeled: DataSet, train_unlabeled: DataSet, test: DataSet, valid: DataSet,
                 height=32, width=32, colors=3, nclass=2, mean=0, std=1, p_labeled=None, p_unlabeled=None):
        self.name = name
    ...
    def creator(cls, name, seed, label, valid, augment, parse_fn=record_parse, do_memoize=False,
                nclass=2, colors=3, height=32, width=32):
        if not isinstance(augment, list):
            augment = augment(name)
        fullname = f'.{seed}@{VALIDATION_SAMPLES}'
        ...
        def create():
            ...
            train_labeled = DataSet.from_files(
                [f'{root}{fullname}.tfrecord'], augment[0], parse_fn, image_shape)
            train_unlabeled = DataSet.from_files(
                [f'{DATA_DIR}/fixmatch_unlabel{fullname}.tfrecord'], augment[1], parse_fn, image_shape)
            if do_memoize:
            ...
            test_data = DataSet.from_files(
                [os.path.join(DATA_DIR, f'fixmatch_test{fullname}.tfrecord')], NOAUGMENT, parse_fn, image_shape=image_shape)
...
def create_datasets(augment_fn):
    d = {}
    d.update([DataSets.creator('fixmatch_train', seed, label, valid, augment_fn, height=INPUT_SHAPE[0], width=INPUT_SHAPE[1])
              for seed, label, valid in itertools.product([SEED], [2 * x for x in SAMPLES_PER_CLASS], [VALIDATION_SAMPLES])]) # range(6) [1, 5000]
    return d
